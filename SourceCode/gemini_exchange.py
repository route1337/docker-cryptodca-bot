...
def get_coin_price(api_url: str, currency: str) -> float:
    ...
    try:
        ...
        logging.error("Unable to get price due to %s", err)
        logging.error("Price feed: %s", price_feeds)
    return coin_price
...
def verify_balance(api_url: str, config_file: str, buy_amount: float) -> bool:
    ...
    try:
        ...
        logging.error("Unable to get current balance!")
        logging.error(err)
        return False
...
def get_decimal_max(api_url: str, currency: str) -> int:
    ...
    """
    # Instantiate Gemini and query the price
    api_query = f"/v1/symbols/details/{currency}usd".lower()
...
def buy_currency(api_url: str, config_file: str,
    ...
    if 'executed_amount' in order_result.keys():
        logging.info("Buy order succeeded.")
        logging.info("Buy Results: %s", json.dumps(order_result, indent=2))
        return True
    logging.error("Buy order failed.")
    logging.error("Reason: %s", json.dumps(order_result, indent=2))
    return False
