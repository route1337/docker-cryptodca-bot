...
def get_coin_price(api_url: str, config_file: str, currency: str) -> float:
    ...
    api_query = f"products/{currency}-USD/ticker"
...
def buy_currency(api_url: str, config_file: str, currency: str, buy_amount: float) -> bool:
    ...
    order_config = json.dumps({'type': 'market', 'funds': buy_amount,
                               'side': 'buy', 'product_id': f'{currency}-USD'})
    ...
    if 'message' in buy_result:
        ...
        logging.error("Reason: %s", buy_result['message'])
        return False
    logging.info("Buy order succeeded.")
    logging.info("Buy Results: %s", json.dumps(buy_result, indent=2))
    return True
